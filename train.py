...
class Trainer:
    def __init__(self, conf):
        ...
        self.data_loaders, self.data_sizes, self.classids_labels = get_data_loaders(conf)
    ...
    def _init_model_param(self):
        ...
        self.optimizer = optim.Adam(self.model.parameters(), lr=self.conf.lr, weight_decay=5e-4)
        self.schedule_lr = optim.lr_scheduler.MultiStepLR(self.optimizer, self.conf.milestones, self.conf.gamma, -1)
    ...
    def _train_stage(self):
        ...
        for e in range(self.conf.epochs):
            ...
            with torch.no_grad():
                ...
                validation_acc = float(validation_corrects.double() / self.data_sizes['val'])
                ...
                if validation_acc >= best_acc and test_acc >= best_acc_test:
                    best_model_path = os.path.join(self.conf.model_path, self.conf.job_name + '_best_model.pth')
            ...
            plt.savefig(os.path.join(self.conf.model_path, f"Accuracy_{self.conf.job_name}.png"))
            plt.close()
    ...
    def _define_network(self):
        if self.conf.net == 'resnet50':
            ...
            model.fc = Sequential(model.fc, Linear(in_features=1000, out_features=self.conf.nb_classes, bias=True))
            ...
            model.load_state_dict(checkpoint, strict=False)
            model.classifier.add_module(str(len(model.classifier)), Linear(in_features=1000, out_features=self.conf.nb_classes, bias=True))
...
def parse_args():
    ...
    parser.add_argument('--network', type=str, default='resnet18', help='[resnet18, resnet34, resnet50, mobilenet_v2, mobilenet_v3_small]')
    ...
    parser.add_argument('--dataset', type=str, default='dataset/mvtec', help="Path dataset")
    parser.add_argument('--path_pretrain', type=str, default='None', help="Path weight pretrain [None, path_weights]")
    parser.add_argument("--job_name", type=str, default="resnet18", help="The name of experiment")
